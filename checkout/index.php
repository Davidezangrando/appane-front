...
<h1 class="mb-4"><i class="bi bi-credit-card"></i> Conferma Ordine</h1>
...
<div class="row g-4">
    ...
    <div class="col-lg-5">
        <div class="card dash-card">
            <div class="card-body">
                <h5 class="card-title mb-3">Dati di consegna</h5>
                <form action="<?= SITE_URL ?>/checkout/conferma.php" method="POST">
                    <?= csrfField() ?>

                    <div class="row g-2 mb-3">
                        <div class="col-8">
                            <label class="form-label">Via / Piazza *</label>
                            <input type="text" class="form-control" name="nome_via_consegna" value="<?= sanitize($user['NomeVia'] ?? '') ?>" required>
                        </div>
                        <div class="col-4">
                            <label class="form-label">N. civico *</label>
                            <input type="text" class="form-control" name="numero_civico_consegna" value="<?= sanitize($user['NumeroCivico'] ?? '') ?>" required>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">CAP *</label>
                        <input type="text" class="form-control" name="cap_consegna" value="<?= sanitize($user['CAP'] ?? '') ?>" maxlength="10" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Telefono di emergenza *</label>
                        <input type="tel" class="form-control" name="telefono_emergenza" value="<?= sanitize($user['NumeroTelefono'] ?? '') ?>" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Indicazioni per la consegna</label>
                        <textarea class="form-control" name="indicazioni" rows="3" placeholder="Es: suonare al secondo piano, lasciare al portiere..."></textarea>
                    </div>
